...
const store = new Vuex.Store({
  ...
  mutations: {
    ...
    decodeToken: (state, token) => {
      // console.log(1, jwt(localStorage.token).username)
  ...
  actions: {
    ...
    api_operation_ticket_action: (context, params) => {
      // console.log(`/api/v1/service/operation_ticket/${params.id}/`)
      return new Promise((resolve, reject) => {
        axios.post(`/api/v1/service/operation_ticket/${params.id}/`).then(resp => {
          resolve(resp)
        }).catch(error => {
          reject(error)
        })
